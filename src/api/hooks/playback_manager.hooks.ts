...
export function usePlaybackSyncCurrentProgress(fallbackMediaId?: number) {
    ...
    return useServerMutation<number>({
        ...
        onSuccess: async mediaId => {
            const targetMediaId = typeof mediaId === "number" && mediaId > 0 ? mediaId : fallbackMediaId

            if (targetMediaId) {
                await queryClient.invalidateQueries({ queryKey: [API_ENDPOINTS.ANIME_ENTRIES.GetAnimeEntry.key, String(targetMediaId)] })
            }
